...
export const getRecentActivityLogs = asyncHandler(async (req: Request, res: Response) => {
  ...
  const parsedLimit = parsePositiveIntOrThrow(limit, 10, "limit must be a positive integer.");
...
export const getGoalStreaks = asyncHandler(async (req: Request, res: Response) => {
  ...
  const streaks = await dashboardService.getGoalStreaks(userId);
  
  res.status(200).json({ 
    success: true, 
    message: "Goal streaks fetched successfully", 
    data: streaks,
  });
});
...
export const suggestNextAction = asyncHandler(async (req: Request, res: Response) => {
  const userId = getUserId(req);
  
  const result = await dashboardService.suggestNextAction(userId);
  
  res.status(200).json({ 
    success: true, 
    message: "Next action suggested successfully", 
    data: result 
  });
});
...
export const suggestUpskillingResource = asyncHandler(async (req: Request, res: Response) => {
  const userId = getUserId(req);
  
  const result = await dashboardService.suggestUpskillingResource(userId);
  
  res.status(200).json({ 
    success: true, 
    message: "Upskilling resource suggested successfully", 
    data: result 
  });
});
